...
- Диалоги с агентами: можно открывать общую ленту сообщений и диалоги по конкретным агентам, отправлять команды и получать ответы в реальном времени.
- Общие агенты для всех пользователей: агенты находятся в единой симуляции, поэтому любой пользователь, который открыл приложение, взаимодействует с теми же агентами и видит общие изменения.
- Граф связей и события: в интерфейсе доступны визуализация связей между агентами, лента событий и обновляемая статистика по состоянию мира.
- Управление симуляцией: можно менять скорость времени, отправлять групповые команды и перезапускать симуляцию.