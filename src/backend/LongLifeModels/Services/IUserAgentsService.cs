...
public interface IUserAgentsService
{
    ...
    Task<PagedResultDto<AgentMessageDto>> GetMessagesAsync(string userId, int limitPerAgent, CancellationToken cancellationToken);
    ...
    Task<BroadcastCommandAckDto> BroadcastCommandAsync(string userId, BroadcastAgentCommandRequestDto request, CancellationToken cancellationToken);
